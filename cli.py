"""
Command Line Interface for Kidney Transplant Kpi Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="kidney-transplant-kpi-agent", description="Kidney Transplant Kpi Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  KIDNEY TRANSPLANT KPI AGENT")
        print(f"  Domain: Nephrology & Dialysis Systems | Standard: KDIGO / KDOQI Clinical Practice Guidelines")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Kidney Transplant Kpi Agent Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        ...
        if not rows:
            print(f"Warning: No data rows found in '{args.input}'.", file=sys.stderr)
            return 1
        ...
        for idx, r in enumerate(rows, start=1):
            try:
                payload = SystemTaskPayload(
                    task_id=r.get("task_id", f"TASK-{idx:03d}"),
                    target_identifier=r.get("target_identifier", f"TARGET-{idx:03d}"),
    ...
    if args.command == "serve":
        ...
        print(f"Starting Kidney Transplant Kpi Agent API server on http://{args.host}:{args.port}")
