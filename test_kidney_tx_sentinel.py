import pytest
from kidney_tx_sentinel import KDPIProfileCalculatorAgent, DGFRiskForecasterAgent, BanffBiopsyClassifierAgent, KidneyTxCoordinator, main


def test_sub_agents():
    a1 = KDPIProfileCalculatorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = DGFRiskForecasterAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = BanffBiopsyClassifierAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = KidneyTxCoordinator()
...
def test_domain_registry():
    from kidney_tx_sentinel import DomainKnowledgeRegistry
